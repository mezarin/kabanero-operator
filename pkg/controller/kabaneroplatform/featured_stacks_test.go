...
var fstlog = logf.Log.WithName("featured_stacks_test")
var featuredTestLogger logr.Logger = fstlog.WithValues("Request.Namespace", "test", "Request.Name", "featured_stacks_test")
...
func TestResolveFeaturedStacksTwoRepositories(t *testing.T) {
	...
	stacks, err := featuredStacks(k, cl)
	if err != nil {
		t.Fatal("Could not resolve the featured stacks from the default index", err)
	}
